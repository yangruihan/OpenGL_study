...
Window window(480, 480, "test7_material");
...
int main()
{
    ...
    auto light_ambient_factor = glm::vec3(0.3f);
    auto light_diffuse_factor = glm::vec3(0.8f);

    obj_shader.set_vec3f("u_Light.ambient", light_color * light_ambient_factor);
    obj_shader.set_vec3f("u_Light.diffuse", light_color * light_diffuse_factor);
    ...
    while (window.show())
    {
        ...
        light_color.x = glm::sin(glfwGetTime() * 1.2f);
        light_color.y = glm::cos(glfwGetTime() * 1.0f);
        light_color.z = glm::sin(glfwGetTime() * 0.8f);
